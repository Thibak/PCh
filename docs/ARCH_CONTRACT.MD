...
├── .github/                
│   └──  workflows/
│       └── main.yml          # PCh CI/CD Pipeline (GitHub Actions)
│
...
│
├── lib/                      # Внешние и внутренние библиотеки (управляются PlatformIO)
│   ├── Arduino-BLE-MIDI/     # (Например)
│   │
│   └── mocks/                # Библиотека Mock-объектов для тестирования
│       ├── MockEventHandler.h  # Spy для тестирования событий
...
│   ├── test_config_manager/  # Unit-тесты для ConfigManager
│   │   └── test_main.cpp
│   │
│   ├── test_event_dispatcher/ # Unit-тесты для EventDispatcher
│   │   └── test_main.cpp
│   │
│   ├── test_scheduler/       # Интеграционные тесты Scheduler
│   │   └── test_main.cpp
│   │
│   └── ... (другие тесты)
│
...
## **6\. Стратегия и стандарты тестирования**
### **6\.1\. Уровни тестирования**

* Unit-тесты (Host / `env:native`):
  * Тестирование чистых функций и классов в изоляции.
  * Используются Mock-объекты (из `lib/mocks/`) для всех HAL-зависимостей.
  * Запускаются локально и в CI/CD.
  * Цель: Проверка бизнес-логики, парсинга, алгоритмов (DSP).
* Интеграционные тесты (Host / `env:native`):
  * Тестирование взаимодействия нескольких модулей APP и CORE (цепочка событий).
  * Используются Mock-объекты для HAL.
  * Пример: `Sensors` -\> `Logic` -\> `Fingering` -\> `Midi`.
  * Цель: Проверка контрактов между модулями и потока данных.
* Unit-тесты HAL (Target / `env:test_hal_...`):
  * Изолированное тестирование реальных HAL-драйверов на железе.
  * Пример: Проверка чтения файла из реального SPIFFS, проверка мигания LED.
  * Цель: Проверка корректности интеграции с ESP-IDF/Arduino SDK.
* End-to-End тесты (Target / `env:e2e_tests`):
  * Полные HIL-тесты (Hardware-in-the-Loop) на реальном устройстве.
  * Цель: Финальная валидация системы.

### **6\.2\. Правила написания тестов (Host-First)**
* Безопасность конфигурации: Тесты, работающие с файловой системой (например, через `MockHalStorage` с реальным доступом к диску ПК), не должны перезаписывать пользовательские файлы (`data/*.cfg`).
  * Используйте паттерн Backup/Restore в функциях setUp/tearDown.
  * Перед тестом: если файл существует, переименовать его в `.bak`.
  * После теста: удалить тестовый файл и восстановить `.bak`.
* Unity Fixtures:
  * Функции `setUp` и `tearDown` объявляются без использования `extern "C"`.
* Структура тестов в PlatformIO:
  * Используется стратегия "Один тест-сьют — одна папка".
  * Каждый набор тестов находится в отдельной подпапке внутри `test/` (напр., `test/test_config_manager/`).
  * Каждая папка содержит свой файл `test_main.cpp` с функцией `main()`, которая вызывает `UNITY_BEGIN()` и `RUN_TEST()`.
  * Это гарантирует корректную линковку и изоляцию тестов.